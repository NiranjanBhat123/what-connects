"""
Enhanced admin configuration for games app - FIXED FOR PENDING STATUS
"""
...
class GameAdmin(TimeStampedAdmin):
    """Enhanced admin interface for Game model with PENDING status support."""
    ...
    def status_display(self, obj):
        """Display status with color - INCLUDES PENDING."""
        colors = {
            'pending': '#FF9800',  # Orange for pending
            'active': '#2196F3',   # Blue for active
    ...
    def progress_display(self, obj):
        ...
        # For pending games, show "Pre-generated"
        if obj.status == 'pending':
            return format_html(
                '<span style="color: #FF9800; font-weight: bold;">⏳ Pre-generated ({} questions ready)</span>',
                total
            )
    ...
    def player_count(self, obj):
        ...
        if obj.status == 'pending':
            # For pending games, show room player count
            room_player_count = obj.room.players.count()
            return format_html('<strong>{}</strong> players (waiting)', room_player_count)
    ...
    def duration(self, obj):
        ...
        if obj.status == 'pending':
            ...
            minutes = delta.total_seconds() / 60
            return f"▶️ {minutes:.1f} min (in progress)"
        return "-"
    ...
    def game_stats(self, obj):
        ...
        # For pending games, show different stats
        if obj.status == 'pending':
            return format_html(
                '<div style="background: #FFF3E0; padding: 15px; border-radius: 5px; border: 2px solid #FF9800;">'
                '<p style="margin: 0; color: #E65100;"><strong>⏳ Pre-generated Game (Ready to Start)</strong></p>'
                '<p style="margin: 8px 0 0 0;"><strong>Questions Ready:</strong> {}</p>'
                '<p style="margin: 4px 0 0 0;"><strong>Room Players:</strong> {}</p>'
                '<p style="margin: 4px 0 0 0; color: #666;"><small>Game will activate when host clicks "Start Game"</small></p>'
                '</div>',
                obj.total_questions,
                obj.room.players.count()
            )
...
class QuestionAdmin(TimeStampedAdmin):
    ...
    list_display = ['question_number', 'game_code', 'game_status', 'items_preview', 'options_count', 'has_hint', 'time_limit', 'answer_stats', 'created_at']
    ...
    def game_status(self, obj):
        ...
        return format_html(
            ...
            colors.get(obj.game.status, 'gray'),
            icons.get(obj.game.status, '❓')
        )
    game_status.short_description = 'Status'
    ...
    def answer_stats(self, obj):
        ...
        if obj.game.status == 'pending':
            return format_html('<span style="color: #FF9800;">⏳ Not started</span>')
    ...
    def answer_statistics(self, obj):
        ...
        if obj.game.status == 'pending':
            return format_html(
                '<div style="background: #FFF3E0; padding: 15px; border-radius: 5px; border: 2px solid #FF9800;">'
                '<p style="color: #E65100; font-weight: bold;">⏳ Pre-generated Question</p>'
                '<p>No answers yet - game has not started</p>'
                '</div>'
            )
...
class AnswerAdmin(TimeStampedAdmin):
    """Enhanced admin interface for Answer model with CORRECT points system."""
    ...
    readonly_fields = ['id', 'points_earned', 'correctness_display', 'points_explanation', 'created_at', 'updated_at']
    date_hierarchy = 'created_at'
    ...
    fieldsets = (
        ...
        ('Performance', {
            'fields': ('time_taken', 'used_hint', 'points_earned', 'points_explanation')
        }),
        ('Timestamps', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
    ...
    def points_display(self, obj):
        """Display points with color - CORRECT SYSTEM."""
        color = 'green' if obj.points_earned > 0 else 'red' if obj.points_earned < 0 else 'gray'
        return format_html(
            '<span style="color: {}; font-weight: bold; font-size: 16px;">{:+d}</span>',
            color, obj.points_earned
        )
    points_display.short_description = 'Points'

    def points_explanation(self, obj):
        """Explain the points calculation."""
        explanations = {
            10: '✅ Correct answer WITHOUT hint → +10 points',
            5: '✅ Correct answer WITH hint → +5 points',
            0: '❌ Wrong answer WITHOUT hint → 0 points (no penalty)',
            -5: '❌ Wrong answer WITH hint → -5 points (penalty)',
        }

        explanation = explanations.get(obj.points_earned, 'Unknown scoring')

        bg_color = '#d4edda' if obj.points_earned > 0 else '#f8d7da' if obj.points_earned < 0 else '#f8f9fa'
        text_color = '#155724' if obj.points_earned > 0 else '#721c24' if obj.points_earned < 0 else '#6c757d'

        return format_html(
            '<div style="background: {}; padding: 10px; border-radius: 5px; color: {};">'
            '<strong>{}</strong>'
            '</div>',
            bg_color, text_color, explanation
        )
    points_explanation.short_description = 'Points System'
...
class GameScoreAdmin(TimeStampedAdmin):
    ...
    list_display = ['rank_display', 'player_name', 'game_code', 'game_status', 'score_display', 'answers_display', 'hints_display', 'accuracy_display', 'created_at']
    ...
    def score_breakdown(self, obj):
        """Detailed score breakdown with CORRECT points system explanation."""
        if not obj.id:
            return '-'
        ...
        points_from_correct = obj.correct_answers * 10  # Assuming all correct without hint
        # Note: This is an approximation. Real calculation would need to check each answer

        return format_html(
            '<div style="background: #f8f9fa; padding: 15px; border-radius: 5px;">'
            '<h3 style="margin-top: 0;">Score Breakdown</h3>'

            '<div style="background: #e3f2fd; padding: 10px; margin-bottom: 10px; border-radius: 3px;">'
            '<strong>Points System:</strong><br/>'
            '<small>✅ Correct WITHOUT hint: +10 pts</small><br/>'
            '<small>✅ Correct WITH hint: +5 pts</small><br/>'
            '<small>❌ Wrong WITHOUT hint: 0 pts</small><br/>'
            '<small>❌ Wrong WITH hint: -5 pts</small>'
            '</div>'
            ...
            '<tr><td style="padding: 8px;">Total Score</td><td style="padding: 8px; text-align: right; font-weight: bold; color: #4CAF50; font-size: 18px;">{}</td></tr>'
